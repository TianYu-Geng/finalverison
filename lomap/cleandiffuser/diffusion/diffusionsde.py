...
def epstheta_to_xtheta(x, alpha, sigma, eps_theta):
    """
    将“噪声预测” eps_theta 转换为“数据/干净样本预测” x_theta（常对应 x0 的估计）。
    这里使用的是线性高斯扩散的关系：x = alpha * x0 + sigma * eps
    若模型预测 eps_theta ≈ eps，则可解出 x0（或 x_theta）：
      x_theta = (x - sigma * eps_theta) / alpha
    """
    # x: 当前噪声状态 x_t（张量），shape 可为 (B,T,D) 等
    # alpha: 对应时间步的系数（通常是 sqrt(alpha_bar_t) 的某种定义），需能 broadcast 到 x
    # sigma: 对应时间步的噪声系数（通常是 sqrt(1-alpha_bar_t)），需能 broadcast 到 x
    # eps_theta: 网络预测的噪声项 ε_θ(x_t, t)
    # 返回：网络对应的“干净样本/数据预测” x_theta（≈ x0）
    return (x - sigma * eps_theta) / alpha  # 按 x = alpha*x0 + sigma*eps 反解 x0


def xtheta_to_epstheta(x, alpha, sigma, x_theta):
    """
    将“数据/干净样本预测” x_theta 转换为“噪声预测” eps_theta。
    同样来自：x = alpha * x0 + sigma * eps
    若模型预测 x_theta ≈ x0，则可解出 eps：
      eps_theta = (x - alpha * x_theta) / sigma
    """
    # x: 当前噪声状态 x_t
    # alpha: 时间步系数（与上面一致）
    # sigma: 噪声系数（与上面一致）
    # x_theta: 网络预测的“干净样本/数据” x_θ(x_t,t)（≈ x0）
    # 返回：对应的噪声项 ε_θ(x_t,t)（≈ eps）
    return (x - alpha * x_theta) / sigma  # 按 x = alpha*x0 + sigma*eps 反解 eps
