'''
在“D4RL 原始离线数据 → ReplayBuffer”之间，对 dataset 字典做可选的、可组合的结构性预处理。
'''
import gym
import numpy as np
import einops
...
def compose(*fns):
    ## 按顺序执行 fns 列表里的函数，每个函数都必须是 fn(x)->x
    def _fn(x):
        for fn in fns:
            x = fn(x)
        return x
...
def get_preprocess_fn(fn_names, env):
    # 根据配置和环境名，返回一个“处理每条轨迹episode的函数”
...
def add_deltas(env):
    ## 输入的dataset包含'observations'和'next_observations'，输出的dataset多了'deltas'
    def _fn(dataset):
...
def maze2d_set_terminals(env):
    ...
    goal = np.array(env._target) # maze2d 环境的目标位置
    threshold = 0.5 # maze2d 环境的目标阈值

    def _fn(dataset):
        xy = dataset['observations'][:,:2]
        distances = np.linalg.norm(xy - goal, axis=-1)
        at_goal = distances < threshold # at_goal[t] 表示第 t 步是否在目标附近
        timeouts = np.zeros_like(dataset['timeouts']) # timeouts[t] 表示第 t 步是否是超时终止

        '''
        这行是核心逻辑：
        把“到达目标的那一步”标成 timeout：
            •	t 时刻在 goal 附近
            •	t+1 时刻不在 goal 附近
            •	说明 t 是一个“终止边界”（一条轨迹在此结束）
        也就是：在离线transition流，用“到达目标并离开”来切分轨迹
        '''
        timeouts[:-1] = at_goal[:-1] * ~at_goal[1:]

        # 找到所有切点，并计算相邻切点之间的路径长度
        ...
        dataset['timeouts'] = timeouts # sequence_dataset 会用这个 timeouts 来切 episode，直接决定你的轨迹数据长什么样。
        return dataset
